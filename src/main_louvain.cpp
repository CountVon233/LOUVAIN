...
unsigned short id_qual = 0; // 0:modularity, 10:suminc
int max_level = 2; // -a: iterative count
int max_comm_size = 1e8; // -m: community max number
long double beta = 0;
...
main(int argc, char **argv) {
  ...
  do {
    ...
    // if(level == 0){
    //   // 第一轮用modularity
    //   int save_id_qual = id_qual;
    //   id_qual = 0;
    //   init_quality(&g, nb_calls);
    //   id_qual = save_id_qual;
    // }

    improvement = c.one_level(max_comm_size);
    ...
    if (filename_part!=NULL && level==1) // do at least one more computation if partition is provided
      improvement=true;
  } while(improvement && level < max_level); // max_level=20
  
  time(&time_end);
  if (verbose) {
    display_time("End");
    cerr << "\nTotal duration: " << (time_end-time_begin) << " sec" << endl;
  }
  cerr << "new_qual=" << new_qual << endl;
  cerr << "all cost time=" << (clock()-s_time)/1e6 << endl;
